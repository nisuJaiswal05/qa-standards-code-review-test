...
## Scope: \*_/_.{ts,js,tsx,jsx} (All TypeScript / JavaScript Files)
...
3. **Explain the rule**: Every comment must reference the specific guideline it enforces (e.g., _"qa-coding-guidelines: No hardcoded delays — use `waitForSelector` instead."_).
4. **Don't duplicate passing checks**: If the existing workflow (`qa-standards-check.yml`) already flags an issue with CI annotations/warnings, note it but do not re-raise it as a blocking comment.
